...
async function start(page){await page.goto(base);await page.locator('#newGameBtn').click();await page.selectOption('#aiCount','0');await page.selectOption('#guidedTutorial','off');await page.locator('#startGameBtn').click();await page.keyboard.press('Escape');}
...
test('offline restart restores save and renders city and gameplay',{tag:'@offline'},async({page,context,browserName})=>{
 await start(page);await page.locator('.bottom-bar [data-view="businesses"]').click();await page.locator('[data-action="open-buy"]').click();await page.locator('[data-buy="machines"]').click();
 ...
 await context.setOffline(true);await page.reload();await page.locator('#continueBtn').click();
 ...
 await page.locator('#endTurnBtn').click();await expect(page.locator('#roundLabel')).toContainText('2');await context.setOffline(false);
});
