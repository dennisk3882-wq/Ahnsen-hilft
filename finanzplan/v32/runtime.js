...
(function(){
  ...
  window.finanzplanV3Start=async function(options={}){await baseStart(options);stamp();let recategorized=0;try{recategorized=FinanzCategoryIntelligence?.reclassifyImportedTransactions?.({onlySuspicious:true})||0;FinanzCategoryIntelligence?.repairLearnedRules?.()}catch(e){FinanzMonitoring?.record?.('intelligence','warn','category_repair_failed',e.message)}try{FinanzIntelligence.learnFromHistory()}catch(e){FinanzMonitoring?.record?.('intelligence','warn','merchant_learning_failed',e.message)}try{if(data.integrations?.cloud?.e2ee?.enabled&&!FinanzE2EE.unlocked())FinanzMonitoring?.record?.('security','info','e2ee_locked','Cloud-E2EE ist auf diesem Gerät gesperrt')}catch{}if(!data.settings?.vault)await storage.saveState(data).catch(()=>{});else if(window.__vaultKey)await persistVault().catch(()=>{});renderAll();stamp();if(recategorized)FinanzMonitoring?.record?.('intelligence','info','categories_repaired',`${recategorized} importierte Buchungen neu kategorisiert`);FinanzMonitoring?.record?.('runtime','info','v32_boot','Finanzplan V3.2 gestartet')};
