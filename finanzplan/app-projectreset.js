...
function emptyProjectData(preservePreferences=true){
  const previousSettings=preservePreferences&&data?.settings?{...data.settings}:null,fresh=defaultData();
  fresh.schemaVersion=V2_SCHEMA;fresh.members=[{id:'m1',name:'Ich',role:'admin',active:true}];fresh.accounts=[];fresh.transactions=[];fresh.recurring=[];fresh.budgets=[];fresh.goals=[];fresh.reserves=[];fresh.contracts=[];fresh.insurances=[];fresh.debts=[];fresh.projects=[];fresh.monthClosures=[];fresh.documents=[];fresh.notifications=[];fresh.trash=[];fresh.audit=[];fresh.merchantRules=[];fresh.reconciliations=[];fresh.subscriptionSuggestions=[];fresh.sync={endpoint:'',token:'',lastSync:null,deviceId:uid('device')};fresh.assistantLog=[{role:'bot',text:'Hallo! Dein Finanzplan ist noch leer. Sobald du Einnahmen und Ausgaben erfasst hast, kann ich deine lokalen Finanzdaten analysieren.'}];
...
function injectDemoBanner(){if(!isOriginalDemoDataset()||sessionStorage.getItem('finanzplan:hide-demo-banner')==='1')return;const root=$('#view-dashboard');if(!root||$('#demoDataBanner',root))return;root.insertAdjacentHTML('afterbegin','<article id="demoDataBanner" class="neo-banner" style="margin-bottom:16px"><h2>Du siehst noch Beispieldaten</h2><p>Die angezeigten Werte sind nur Demo-Daten. Starte hier deinen eigenen leeren Haushaltsplan.</p><div class="action-row"><button id="startOwnProject" class="primary-button">Eigene Finanzen starten</button><button id="hideDemoBanner" class="secondary-button">Später</button></div></article>');$('#startOwnProject',root).onclick=()=>openNewProjectWizard(false);$('#hideDemoBanner',root).onclick=()=>{sessionStorage.setItem('finanzplan:hide-demo-banner','1');$('#demoDataBanner',root)?.remove()}}
const _renderMoreForProjectReset=renderMore;renderMore=function(){_renderMoreForProjectReset();injectProjectResetControls()};const _renderSettingsForProjectReset=renderSettings;renderSettings=function(){_renderSettingsForProjectReset();injectProjectResetControls()};const _renderDashboardForProjectReset=renderDashboard;renderDashboard=function(){_renderDashboardForProjectReset();injectDemoBanner()};
const isFirstRun=sessionStorage.getItem('finanzplan:first-run')==='1';if(isFirstRun){sessionStorage.removeItem('finanzplan:first-run');data=emptyProjectData(false);migrateV2Data();if(!data.settings.vault)localStorage.setItem(STORE_KEY,JSON.stringify(data));renderAll();setTimeout(()=>openNewProjectWizard(true),160)}else{injectProjectResetControls();injectDemoBanner()}
