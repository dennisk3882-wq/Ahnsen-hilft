...
// The legacy synchronous core receives the already loaded IndexedDB state through a
// short-lived in-memory getItem bridge. No plaintext V3 state is written to LocalStorage.
const nativeGetItem=Storage.prototype.getItem;
let bootBridge=!!boot.state&&!vaultExists();
if(bootBridge){
  const serialized=JSON.stringify(boot.state);
  Storage.prototype.getItem=function(key){if(this===localStorage&&key==='finanzplan:data:v1')return serialized;return nativeGetItem.call(this,key)};
}
...
try{
  for(const src of scripts){
    ...
    if(src.endsWith('/app-core.js'))closeBootBridge();
    if(src.endsWith('/app-security.js')){window.__finanzplanLegacyInit=window.init;window.init=()=>{window.__v3InitRequested=true}}
  }
  closeBootBridge();if(!vaultExists())localStorage.removeItem('finanzplan:data:v1');
  ...
  closeBootBridge();console.error('Finanzplan V3 bootstrap failed',err);
  const host=document.getElementById('toastHost');if(host){const el=document.createElement('div');el.className='toast error';el.textContent=`Finanzplan konnte nicht vollständig starten: ${err.message}`;host.appendChild(el)}
}
