...
const scripts=[
  ...
  './v32/intelligence.js','./v32/category-intelligence.js','./v32/category-learning.js','./v32/merchant-brand.js','./v32/merchant-brand-hydration.js','./v32/merchant-brand-final.js','./v32/banking-hub.js','./v32/ai-plus.js','./v32/e2ee.js','./v32/e2ee-guard.js','./v32/monitoring.js','./v32/onboarding.js','./v32/status.js','./v32/runtime.js','./v32/sparkasse.js','./v32/auth-gate.js'
];
...
try{for(const src of scripts){await loadClassic(src);if(src.endsWith('/app-core.js'))closeBootBridge();if(src.endsWith('/app-security.js')){window.__finanzplanLegacyInit=window.init;window.init=()=>{window.__v3InitRequested=true}}}closeBootBridge();if(!vaultExists())localStorage.removeItem('finanzplan:data:v1');if(typeof window.finanzplanV3Start!=='function')throw new Error('V3.2 runtime fehlt');if(typeof window.FinanzAuthGate?.requireSession==='function')await window.FinanzAuthGate.requireSession();await window.finanzplanV3Start({bootState:boot.state,migrated:boot.migrated});if(typeof window.FinanzAuthGate?.afterStart==='function')await window.FinanzAuthGate.afterStart()}catch(err){closeBootBridge();console.error('Finanzplan V3.2 bootstrap failed',err);const host=document.getElementById('toastHost');if(host){const el=document.createElement('div');el.className='toast error';el.textContent=`Finanzplan konnte nicht vollständig starten: ${err.message}`;host.appendChild(el)}}