...
(function(){
  ...
  accountBalanceAtDate=function(aOrId,dateISO=localISO(now)){
    const a=typeof aOrId==='string'?data.accounts.find(x=>x.id===aOrId):aOrId;if(!a)return 0;
    dateISO=FinanceLib.normalizeDate(dateISO)||localISO(now);if(a.openingDate&&dateISO<a.openingDate)return 0;
    const rec=relevantReconciliation(a.id,dateISO),covered=rec?.coveredIds?new Set(rec.coveredIds):null;let base=rec?field(rec,'balance'):field(a,'openingBalance'),from=rec?rec.date:a.openingDate;
    for(const t of data.transactions){const td=FinanceLib.normalizeDate(t.date);if(!td||td>dateISO)continue;if(rec){if(td<from)continue;if(td===from&&(covered?covered.has(t.id):true))continue}else if(td<from)continue;base+=impactCents(t,a.id)}
