...
(function(){
  ...
  const toCents=v=>{
    if(Number.isInteger(v)&&Math.abs(v)>1e12)return v;
    const n=typeof v==='string'&&globalThis.FinanceLib?FinanceLib.parseMoney(v):Number(v);
  ...
  globalThis.FinanzMoney={toCents,fromCents,sumCents,field};

  globalThis.syncMoneyFieldsV3=function(){
    for(const t of data.transactions||[])field(t,'amount');
    for(const r of data.recurring||[])field(r,'amount');
    for(const a of data.accounts||[]){field(a,'openingBalance');if(a.baseBalance!=null)field(a,'baseBalance')}
    for(const r of data.reconciliations||[])field(r,'balance');
    for(const b of data.budgets||[])field(b,'amount');
    for(const g of data.goals||[]){field(g,'target');field(g,'current');if(g.allocated!=null)field(g,'allocated')}
    for(const r of data.reserves||[]){field(r,'target');field(r,'current');field(r,'monthly');if(r._v22BaseCurrent!=null)field(r,'_v22BaseCurrent')}
    for(const c of data.contracts||[])field(c,'amount');
    for(const i of data.insurances||[])field(i,'amount');
    for(const d of data.debts||[]){field(d,'principal');field(d,'openingBalance');field(d,'balance');field(d,'rate');for(const x of d.extraPayments||[])field(x,'amount')}
    for(const p of data.projects||[]){field(p,'budget');field(p,'spent')}
    return data;
  };
  ...
  categoryAmountInTx=function(t,ids){
    const sign=t.type==='refund'?-1:t.type==='expense'?1:0;if(!sign)return 0;let cents=0;const splits=splitLines(t);
  ...
  monthSummary=function(d=selectedMonth){
    const tx=txForMonth(d),paid=tx.filter(t=>(t.status||'paid')==='paid'),planned=tx.filter(t=>(t.status||'paid')==='planned');
    const sum=(arr,type)=>arr.filter(t=>t.type===type).reduce((s,t)=>s+field(t,'amount'),0),incomeC=sum(paid,'income'),refundC=sum(paid,'refund'),grossC=sum(paid,'expense'),expenseC=grossC-refundC,plannedIncomeC=sum(planned,'income'),plannedRefundC=sum(planned,'refund'),plannedGrossC=sum(planned,'expense'),plannedExpenseC=plannedGrossC-plannedRefundC,totalIncomeC=incomeC+plannedIncomeC,totalExpenseC=expenseC+plannedExpenseC;
    return {tx,paid,income:fromCents(incomeC),expense:fromCents(expenseC),grossExpense:fromCents(grossC),refunds:fromCents(refundC),balance:fromCents(incomeC-expenseC),plannedIncome:fromCents(plannedIncomeC),plannedExpense:fromCents(plannedExpenseC),totalExpectedIncome:fromCents(totalIncomeC),totalExpectedExpense:fromCents(totalExpenseC),forecast:fromCents(totalIncomeC-totalExpenseC)}
  };
  ...
  projectSpend=function(p){
    const start=FinanceLib.normalizeDate(p.start),end=FinanceLib.normalizeDate(p.end),tag=FinanceLib.normalizeText(p.tag||p.name),cats=expandedCategoryIds(p.categoryIds||[]),within=t=>{const d=FinanceLib.normalizeDate(t.date);return d&&(!start||d>=start)&&(!end||d<=end)},direct=t=>{const original=t.type==='refund'&&t.refundOf?data.transactions.find(x=>x.id===t.refundOf):null;return (t.projectIds||[]).includes(p.id)||(tag&&(t.tags||[]).map(FinanceLib.normalizeText).includes(tag))||(original&&((original.projectIds||[]).includes(p.id)||(tag&&(original.tags||[]).map(FinanceLib.normalizeText).includes(tag))))};let cents=field(p,'spent');for(const t of data.transactions.filter(t=>['expense','refund'].includes(t.type)&&(t.status||'paid')==='paid'&&within(t))){if(direct(t))cents+=(t.type==='refund'?-1:1)*field(t,'amount');else cents+=toCents(categoryAmountInTx(t,cats))}return fromCents(cents)
  };
  ...
  const priorEarmarked=earmarkedTotal;
  earmarkedTotal=function(events=cashflowEvents(localISO(now),localISO(monthEnd(now)))){return fromCents(toCents(priorEarmarked(events)))};
  ...
  debtBalanceAtDate=function(d,dateISO=localISO(now)){
    dateISO=FinanceLib.normalizeDate(dateISO)||localISO(now);const start=FinanceLib.normalizeDate(d.openingDate||d.start)||dateISO;if(dateISO<start)return fromCents(field(d,'openingBalance')||field(d,'principal')||field(d,'balance'));let bal=field(d,'openingBalance')||field(d,'balance')||field(d,'principal'),monthlyRate=num(d.interest)/100/12,cursor=FinanceLib.parseISO(start),end=FinanceLib.parseISO(dateISO),payments=debtPayments(d,dateISO),pi=0;
    while(cursor<=end&&bal>0){const ym=`${cursor.getFullYear()}-${String(cursor.getMonth()+1).padStart(2,'0')}`;bal+=Math.round(bal*monthlyRate);while(pi<payments.length&&String(payments[pi].date).slice(0,7)===ym){bal=Math.max(0,bal-field(payments[pi],'amount'));pi++}cursor=FinanceLib.addMonths(cursor,1)}return fromCents(Math.max(0,bal))
  };
  netWorthAtDate=function(date=localISO(now)){const assets=data.accounts.filter(a=>a.includeNetWorth!==false).reduce((s,a)=>s+toCents(accountBalanceAtDate(a,date)),0),debts=data.debts.reduce((s,d)=>s+toCents(debtBalanceAtDate(d,date)),0);return fromCents(assets-debts)};
