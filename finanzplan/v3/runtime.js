...
(function(){
  ...
  saveData=function(reason='Änderung gespeichert'){
    ...
    if(data.settings?.vault){storage.clearState().catch(()=>{});stripLegacy();if(typeof persistVault==='function')persistVault().catch(e=>console.error('V3 vault persist failed',e))}
    else{queuePersist(data).then(()=>{window.__v3LastPersistedState=clone(data);stripLegacy()}).catch(e=>console.error('IndexedDB save failed',e))}
  ...
  if(baseUnlockVault)unlockVault=async function(pin){const ok=await baseUnlockVault(pin);stampV3();await persistVault();return ok};
  ...
  window.finanzplanV3Start=async function({migrated}={}){
    ...
    if(data.settings?.vault){await storage.clearState().catch(()=>{});if(window.__vaultKey)await persistVault().catch(()=>{})}else{await storage.saveState(data);window.__v3LastPersistedState=clone(data);stripLegacy()}
