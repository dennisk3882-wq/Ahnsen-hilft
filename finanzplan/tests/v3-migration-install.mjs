...
try{
  ...
  await page.reload({waitUntil:'domcontentloaded'});await page.waitForFunction(()=>globalThis.FinanzV3?.version==='3.2.0'&&data.transactions.some(t=>t.id==='legacy_tx'),null,{timeout:12000});
  assert.equal(await page.evaluate(()=>data.version),'3.2.0');assert.equal(await page.evaluate(()=>data.schemaVersion),3);assert.equal(await page.evaluate(()=>data.transactions.find(t=>t.id==='legacy_tx').amountCents),2222);assert.equal(await page.evaluate(()=>localStorage.getItem('finanzplan:data:v1')),null);assert.equal(await page.evaluate(async()=>!!(await window.__finanzplanStorage.loadState()).transactions.find(t=>t.id==='legacy_tx')),true);
  await context.close();
