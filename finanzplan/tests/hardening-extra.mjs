...
async function stableEvaluate(fn){
  for(let attempt=0;attempt<3;attempt++){
    try{return await page.evaluate(fn)}catch(e){
      if(!/Execution context was destroyed|Cannot find context|Target page, context or browser has been closed/i.test(String(e))||attempt===2)throw e;
      await page.waitForLoadState('domcontentloaded',{timeout:12000}).catch(()=>{});
      await page.waitForFunction(()=>globalThis.data&&typeof globalThis.saveData==='function',{timeout:12000});
    }
  }
}
try{
  ...
  const results=await stableEvaluate(()=>{
  ...
  await stableEvaluate(async()=>{await storeFile(new File([new TextEncoder().encode('trash-only')],'trash-only.txt',{type:'text/plain'}),'');const id=data.documents.find(d=>d.name==='trash-only.txt').id;await deleteStoredFile(id)});
  assert.equal(await stableEvaluate(()=>data.documents.some(d=>d.name==='trash-only.txt')),false);assert.equal(await stableEvaluate(()=>data.trash.some(x=>x.collection==='documents'&&x.item?.name==='trash-only.txt')),true);
  await stableEvaluate(()=>openNewProjectWizard(false));await page.locator('#modalTitle').waitFor({state:'visible'});assert.match(await page.locator('#modalTitle').textContent(),/Belege vor Reset sichern/);await page.locator('[data-cancel]').click();
  ...
  const manifest=await stableEvaluate(async()=>await (await fetch('./manifest.webmanifest')).json());assert.ok(manifest.icons.some(i=>i.purpose==='maskable'));
