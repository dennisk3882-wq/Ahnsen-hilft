...
async function run(browserType,name,viewport){
  ...
  try{
    ...
    await page.waitForFunction(()=>globalThis.FinanzV3?.version==='3.0.0');
    assert.equal(await page.evaluate(()=>data.version),'3.0.0');
    ...
    await page.waitForFunction(()=>globalThis.FinanzV3?.version==='3.0.0'&&data.transactions.some(t=>t.id==='cent20'),null,{timeout:12000});
    assert.equal(await page.evaluate(()=>localStorage.getItem('finanzplan:data:v1')),null);
    assert.equal(await page.evaluate(()=>Math.round(monthSummary().expense*100)),30);
