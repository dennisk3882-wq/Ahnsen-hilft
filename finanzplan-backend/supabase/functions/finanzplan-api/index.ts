...
async function eb(path:string,init:RequestInit={}){const token=await ebJwt(),r=await fetch(`https://api.enablebanking.com${path}`,{...init,headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json',...(init.headers||{})}}),text=await r.text();let j:any={};try{j=text?JSON.parse(text):{}}catch{j={raw:text}}if(!r.ok)throw new Error(j.detail||j.message||`Enable Banking ${r.status}`);return j}
...
function normBank(t:any){const amount=Number(t.transaction_amount?.amount??t.amount??0),credit=amount>=0;return{id:String(t.entry_reference||t.transaction_id||t.reference||crypto.randomUUID()),date:String(t.booking_date||t.value_date||t.date||'').slice(0,10),amount:Math.abs(amount),direction:credit?'credit':'debit',merchant:t.creditor?.name||t.debtor?.name||t.merchant_name||'',title:t.remittance_information?.[0]||t.bank_transaction_code||t.creditor?.name||t.debtor?.name||'N26 Umsatz',remittance:Array.isArray(t.remittance_information)?t.remittance_information.join(' · '):String(t.remittance_information||''),reference:String(t.entry_reference||t.transaction_id||'')}}
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(path==='/api/banking/n26/exchange'&&req.method==='POST'){
      ...
      await admin.from('bank_sessions').upsert({user_id:user.id,household_id:st.householdId,provider:'enablebanking',bank:'N26',session_id:sid,accounts,authorized_at:new Date().toISOString(),valid_until:sess.valid_until||null,updated_at:new Date().toISOString()},{onConflict:'user_id,household_id,provider'});
    ...
    if(path==='/api/banking/n26/sync'){
      ...
      for(let i=0;i<20;i++){const q=new URLSearchParams({date_from:from,date_to:to});if(cont)q.set('continuation_key',cont);const j=await eb(`/accounts/${encodeURIComponent(aid)}/transactions?${q}`);txs.push(...(j.transactions||[]));cont=j.continuation_key||'';if(!cont)break}
      const bal=await eb(`/accounts/${encodeURIComponent(aid)}/balances`),balances=bal.balances||bal||[],preferred=balances.find((x:any)=>/closing|interimAvailable|expected/i.test(String(x.balance_type||x.type||'')))||balances[0],balance=Number(preferred?.balance_amount?.amount??preferred?.amount??NaN);
