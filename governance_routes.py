...
from governance import ROLES, case_history, list_admins, save_admin, save_content_revision, set_admin_totp, update_case
...
async def admin_user_totp(request: Request, username: str):
    ...
    secret = set_admin_totp(username, enabled)
    if enabled:
        return HTMLResponse(f"<!doctype html><html lang=de><meta name=viewport content='width=device-width,initial-scale=1'><body style='font:16px system-ui;max-width:680px;margin:auto;padding:28px'><h1>2FA eingerichtet</h1><p>Trage diesen Schlüssel jetzt einmalig in einer kostenlosen Authenticator-App ein:</p><p style='padding:18px;background:#eef5eb;border-radius:12px;font:700 20px monospace;overflow-wrap:anywhere'>{escape(secret)}</p><p>Danach wird beim nächsten Login zusätzlich der sechsstellige Einmalcode verlangt.</p><a href='/intern/benutzer'>Zurück zu den Verwaltungskonten</a></body></html>")
    else:
        message = "2FA wurde abgeschaltet."
    return RedirectResponse("/intern/benutzer?hinweis=" + quote(message), status_code=303)
