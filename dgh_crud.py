...
def init_dgh_db():
    ...
    vorhandene_spalten = {
        spalte["name"] for spalte in inspect(engine).get_columns("dgh_termine")
    }
    migrationen = {
    ...
    for spaltenname, spaltentyp in migrationen.items():
        ...
        with engine.begin() as conn:
            conn.exec_driver_sql(
                f"ALTER TABLE dgh_termine ADD COLUMN {spaltenname} {spaltentyp}"
            )
        print(f"Spalte dgh_termine.{spaltenname} hinzugefügt.")

    with engine.begin() as conn:
        conn.exec_driver_sql(
            "UPDATE dgh_termine SET status = 'Bestätigt' "
            "WHERE status = 'Belegt' OR status IS NULL"
        )
        conn.exec_driver_sql(
            "CREATE INDEX IF NOT EXISTS ix_dgh_termine_pwa_user_id "
            "ON dgh_termine (pwa_user_id)"
        )
...
def get_dgh_termine_fuer_benutzer(user_id):
    db = SessionLocal()
    try:
        return (
            db.query(DGHTermin)
            .filter(DGHTermin.pwa_user_id == user_id)
            .order_by(DGHTermin.erstellt_am.desc())
            .all()
        )
    finally:
        db.close()
    ...
    try:
        ...
        if termin:
            if termin.status == "Bestätigt" and _hat_bestaetigten_konflikt(
                db, datum, ausgenommen_id=termin_id
            ):
                raise ValueError("Für diesen Tag ist bereits ein Termin bestätigt.")
            termin.datum = datum
...
def set_dgh_status(termin_id, status):
    ...
    try:
        ...
        if termin:
            if status == "Bestätigt" and _hat_bestaetigten_konflikt(
                db, termin.datum, ausgenommen_id=termin_id
            ):
                raise ValueError("Für diesen Tag ist bereits ein Termin bestätigt.")
...
def get_freie_tage(anzahl_tage=30):
    ...
    heute = datetime.today().date()
    return [
        heute + timedelta(days=i)
        for i in range(anzahl_tage)
        if heute + timedelta(days=i) not in belegte_daten
    ]
