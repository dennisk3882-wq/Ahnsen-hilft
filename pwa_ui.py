...
def report_page(error: str = "", values: dict | None = None) -> HTMLResponse:
    ...
    consent_checked = " checked" if str(values.get("datenschutz", "")).strip().casefold() == "ja" else ""
...
<form class="report-form" method="post" action="/api/maengel" enctype="multipart/form-data">
...
<section class="form-section"><div class="section-number">2</div><div class="section-copy"><h2>Wo ist der Mangel?</h2><p>Je genauer der Ort, desto schneller kann er geprüft werden.</p></div><label class="field full"><span>Straße, Hausnummer oder Ortsbeschreibung *</span><input name="ort" maxlength="180" required value="{escape(str(values.get('ort', '')))}" placeholder="z. B. Schulstraße 5, gegenüber der Bushaltestelle"></label><div class="location-row"><button class="secondary-button" id="use-location" type="button">{icon('pin')} Standort übernehmen</button><small id="location-status" aria-live="polite">Optional – nur nach deiner Freigabe.</small></div><input id="latitude" name="latitude" type="hidden" value="{escape(str(values.get('latitude', '')))}"><input id="longitude" name="longitude" type="hidden" value="{escape(str(values.get('longitude', '')))}"></section>
<section class="form-section"><div class="section-number">3</div><div class="section-copy"><h2>Kurze Beschreibung</h2><p>Beschreibe, was genau auffällig oder beschädigt ist.</p></div><label class="field full"><span>Beschreibung *</span><textarea name="beschreibung" minlength="10" maxlength="1500" required placeholder="Was ist passiert und seit wann besteht das Problem?">{escape(str(values.get('beschreibung', '')))}</textarea><small>Bitte mindestens 10 Zeichen eingeben.</small></label></section>
...
def more_page(settings: dict) -> HTMLResponse:
    items = [("Meldestatus", "/meldestatus", "search", "Bearbeitungsstand prüfen"), ("Verwaltung", "/verwaltung", "building", "Geschützter Bereich"), ("Barrierefreiheit", "/barrierefreiheit", "shield", "Erklärung und Feedback"), ("Leichte Sprache", "/leichte-sprache", "info", "Die wichtigsten Funktionen einfach erklärt"), ("Datenschutz", "/datenschutz", "shield", "Datenverarbeitung"), ("Impressum", "/impressum", "news", "Anbieterkennzeichnung")]
