...
function vapidJwt(endpoint, privateJwk) {
  ...
  const payload = b64url(JSON.stringify({
    ...
    sub: String(process.env.VAPID_SUBJECT || 'mailto:admin@quiztime.local'),
  }));
...
async function sendEmptyPush(endpoint, keys) {
  const response = await fetch(endpoint, {
    method: 'POST',
    headers: {
      Authorization: `vapid t=${vapidJwt(endpoint, keys.private_jwk)}, k=${publicApplicationKey(keys.public_jwk)}`,
      TTL: '120',
      Urgency: 'normal',
      'Content-Length': '0',
    },
...
async function notifyProfile(profileId, notification) {
  ...
  await Promise.all(subscriptions.map(async subscription => {
    ...
    } catch { /* Push ist optional; In-App-Nachricht bleibt erhalten. */ }
  }));
  return true;
}
...
async function createMatchedRoom(match) {
  const [profileA, profileB] = await Promise.all([
    profileStore.getProfileById(match.profileA),
    profileStore.getProfileById(match.profileB),
  ]);
  ...
  const createdResponse = await fetch(`${base}/api/online/rooms`, {
    ...
    body: JSON.stringify({
      ...
      questionCount: 10,
      maxPlayers: 2,
      team: match.gameMode === 'teams' ? 'violet' : null,
    }),
  });
...
function installPlatformRoutes(app, { requireProfile, profileForRequest }) {
  app.get('/api/platform/status', wrap(async (_req, res) => {
    res.json({ enabled: storage.enabled(), features: ['friends','invites','matchmaking','tournaments','seasons','packs','push','reports','admin','monitoring'] });
  }));

  app.get('/api/platform/me', requireProfile, (req, res) => res.json({ profile: req.soloProfile }));
  app.get('/api/platform/profiles/search', requireProfile, wrap(async (req, res) => res.json({ profiles: await storage.profileSearch(req.soloProfile.id, req.query.q) })));
  ...
  app.post('/api/platform/friends/respond', requireProfile, wrap(async (req, res) => {
    await storage.respondFriend(req.soloProfile.id, String(req.body?.profileId || ''), req.body?.accept === true);
    res.json({ ok: true });
  }));
  ...
  app.post('/api/platform/invites', requireProfile, wrap(async (req, res) => {
    ...
    const invite = await storage.createInvite({
      senderId: req.soloProfile.id, recipientId,
      roomCode: req.body?.roomCode, type: req.body?.type,
      referenceCode: req.body?.referenceCode, message: req.body?.message,
    });
  ...
  app.get('/api/platform/seasons/current', requireProfile, wrap(async (_req, res) => res.json(await storage.seasonLeaderboard(100))));
  ...
  app.get('/api/platform/packs', requireProfile, wrap(async (req, res) => res.json({ packs: await storage.listPacks(req.soloProfile.id) })));
  app.post('/api/platform/packs', requireProfile, wrap(async (req, res) => res.status(201).json({ pack: await storage.createPack(req.soloProfile.id, req.body || {}) })));
  app.get('/api/platform/packs/:code', wrap(async (req, res) => {
    const profile = await profileForRequest(req).catch(() => null);
    const pack = await storage.getPack(req.params.code, profile?.id || null);
    if (!pack) return res.status(404).json({ error: 'Quizpaket nicht gefunden oder nicht freigegeben.' });
    res.json({ pack: { ...pack, owner_id: undefined } });
  }));
  app.post('/api/platform/packs/:code/play', wrap(async (req, res) => { await storage.registerPackPlay(req.params.code); res.json({ ok: true }); }));
  ...
  app.post('/api/platform/matchmaking/join', requireProfile, wrap(async (req, res) => {
    ...
    if (match) {
      createMatchedRoom(match).catch(async error => {
        await storage.setMatchFailed(match.id, error.message);
        await storage.recordMetric({ type: 'matchmaking_error', details: { message: error.message } });
      });
    }
  ...
  app.get('/api/platform/push/public-key', requireProfile, wrap(async (_req, res) => {
    const keys = await storage.getPushKeys();
    res.json({ publicKey: publicApplicationKey(keys.public_jwk) });
  }));
  ...
  app.get('/api/platform/admin/summary', requirePlatformAdmin, wrap(async (_req, res) => res.json(await storage.dashboardSummary())));
