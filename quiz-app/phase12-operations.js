...
async function releaseChecks({ persist = false } = {}) {
  ...
  add('catalog-child', catalogs.child.length >= 1000, 'fail', 'Kinderfragen', `${catalogs.child.length} veröffentlichte Fragen; erforderlich sind mindestens 1000.`);
  add('catalog-adult', catalogs.adult.length >= 1000, 'fail', 'Erwachsenenfragen', `${catalogs.adult.length} veröffentlichte Fragen; erforderlich sind mindestens 1000.`);
