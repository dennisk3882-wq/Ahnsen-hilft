...
let voiceCache = { expiresAt: 0, voices: [], defaultVoiceId: CONFIGURED_VOICE_ID || FALLBACK_VOICE_ID, error: null };
...
async function fetchVoices() {
  if (!API_KEY) return { voices: [], defaultVoiceId: null, error: 'ELEVENLABS_API_KEY fehlt.' };
  ...
  try {
    ...
    voiceCache = { expiresAt: Date.now() + VOICE_CACHE_MS, voices, defaultVoiceId, error: null };
  } catch (error) {
    voiceCache = {
      expiresAt: Date.now() + 5 * 60 * 1000,
      voices: [{ id: CONFIGURED_VOICE_ID || FALLBACK_VOICE_ID, name: 'ElevenLabs-Standardstimme', description: '', gender: '', accent: '', category: '', previewUrl: null }],
      defaultVoiceId: CONFIGURED_VOICE_ID || FALLBACK_VOICE_ID,
      error: `Stimmenliste nicht verfügbar: ${error.message}`,
    };
  }
  return voiceCache;
}
...
function buildSpeechText(question, scope) {
  ...
  if (scope === 'question') return `Frage. ${text}`;
...
async function synthesize({ question, scope = 'all', quizType = 'child', voiceId, ip = 'unknown' }) {
  ...
  const normalizedScope = scope === 'question' ? 'question' : 'all';
    ...
    const response = await fetch(`${API_BASE}/v1/text-to-speech/${encodeURIComponent(selectedVoiceId)}?output_format=mp3_44100_128`, {
      ...
      body: JSON.stringify({
        ...
        voice_settings: {
          ...
          style: quizType === 'child' ? 0.08 : 0.12,
          use_speaker_boost: true,
          speed: quizType === 'child' ? 0.82 : 0.94,
        },
      }),
...
async function getPublicConfig() {
  if (!API_KEY) return { enabled: false, modelId: MODEL_ID, voices: [], defaultVoiceId: null, error: 'API-Key fehlt.' };
