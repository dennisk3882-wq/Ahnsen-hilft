...
function validateCombinedCatalog(questions, { expectedPrefix, label }) {
  assert.strictEqual(questions.length, 2000, `${label}: Gesamtkatalog muss 2.000 Fragen enthalten.`);
  ...
  assert.strictEqual(new Set(questions.map(question => String(question.id).toLocaleLowerCase('de-DE'))).size, 2000, `${label}: doppelte IDs im Gesamtkatalog.`);
  assert.strictEqual(new Set(questions.map(question => normalizeText(question.text))).size, 2000, `${label}: doppelte Texte im Gesamtkatalog.`);
}
...
assert.strictEqual(catalogService.canonicalCatalog('child').length, 2000, 'Katalogservice veröffentlicht nicht alle 2.000 Kinderfragen.');
assert.strictEqual(catalogService.canonicalCatalog('adult').length, 2000, 'Katalogservice veröffentlicht nicht alle 2.000 Erwachsenenfragen.');
...
assert.strictEqual(adultCatalog.meta.expansion3Count, 500, 'Der dritte Erwachsenen-Ausbau enthält nicht exakt 500 Fragen.');
assert.strictEqual(adultCatalog.meta.expansionCount, 1500, 'Alle Erwachsenen-Ausbaustufen enthalten zusammen nicht exakt 1.500 Fragen.');

console.log('QuizTime question expansion batch 1 remains valid inside the 2.000-question catalogs.');
