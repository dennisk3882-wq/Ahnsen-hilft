...
function resetTimerLoop() {
  ...
  if (key !== lastQuestionKey) { lastQuestionKey = key; beepedSeconds = new Set(); }
...
function updateTimers() {
  ...
  if (seconds <= 5 && seconds >= 1 && !beepedSeconds.has(seconds)) { beepedSeconds.add(seconds); beep(seconds); }
}
async function submitAnswer(answerIndex) {
  unlockAudio();
  document.querySelectorAll('[data-answer]').forEach(button => { button.disabled = true; });
  try {
    const response = await fetch('/api/player/answer', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-player-token': playerToken }, body: JSON.stringify({ answerIndex }) });
...
function connectSocket() {
  ...
  socket.on('auth_error', message => { els.loginMessage.textContent = message; localStorage.removeItem('ahnsen_player_token'); setTimeout(() => location.reload(), 700); });
...
async function login() {
  ...
  try {
    const response = await fetch('/api/player/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: els.playerName.value, password: els.eventPassword.value, playerToken }) });
    const data = await response.json();
...
(async () => {
  ...
  if (playerToken) {
    ...
    if (response.ok) { renderState(await response.json()); connectSocket(); }
    else localStorage.removeItem('ahnsen_player_token');
  }
})();
