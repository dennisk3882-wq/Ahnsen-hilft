...
(() => {
  ...
  const CACHE_NAME = 'ahnsen-elevenlabs-audio-v1';
  ...
  function browserFallback(question, reason = '') {
    if (!question || !('speechSynthesis' in window)) return;
    stopSpeech();
    const letters = ['A', 'B', 'C', 'D'];
    const text = preferences.scope === 'question'
    ...
    utterance.rate = question.quizType === 'child' ? 0.84 : 0.94;
  ...
  async function elevenLabsSpeak() {
    ...
    if (!speechConfig?.enabled) {
      browserFallback(question, speechConfig?.error || 'ElevenLabs ist nicht verfügbar.');
      return;
    }
    ...
    try {
      ...
      currentAudio.onerror = () => {
        stopSpeech();
        browserFallback(question, 'Audiodatei konnte nicht abgespielt werden.');
      };
      await currentAudio.play();
    } catch (error) {
      stopSpeech();
      browserFallback(question, error.message);
      showStatus(`ElevenLabs nicht verfügbar – Browserstimme wird verwendet. ${error.message}`, 'warning');
    } finally {
      loading = false;
  ...
  function renderVoiceOptions() {
    ...
    select.disabled = !speechConfig?.enabled || !voices.length;
  }
  ...
  function installControls() {
    ...
    if (speechToggle && !document.querySelector('#elevenLabsControls')) {
      ...
      controls.innerHTML = `
        <div class="elevenlabs-heading"><div><strong>Natürliche ElevenLabs-Stimme</strong><small>Die Frage wird auf dem Server vertont; dein API-Key bleibt verborgen.</small></div><span class="elevenlabs-badge">AI Voice</span></div>
  ...
  async function loadConfig() {
    ...
    try {
      ...
      renderVoiceOptions();
      if (data.enabled) {
        showStatus(data.warning ? `Bereit, mit Hinweis: ${data.warning}` : `ElevenLabs ist bereit · Modell ${data.modelId}`, data.warning ? 'warning' : 'success');
      } else {
