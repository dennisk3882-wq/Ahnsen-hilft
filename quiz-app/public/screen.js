...
function beep(second) { if (!audioContext) return; try { const oscillator = audioContext.createOscillator(); const gain = audioContext.createGain(); oscillator.frequency.value = second === 1 ? 1080 : 790; gain.gain.setValueAtTime(.0001, audioContext.currentTime); gain.gain.exponentialRampToValueAtTime(.16, audioContext.currentTime + .01); gain.gain.exponentialRampToValueAtTime(.0001, audioContext.currentTime + .13); oscillator.connect(gain).connect(audioContext.destination); oscillator.start(); oscillator.stop(audioContext.currentTime + .14); } catch {} }
function esc(value) { return String(value ?? '').replace(/[&<>'"]/g, char => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' }[char])); }
function clock(ms) { const seconds = Math.max(0, Math.ceil(ms / 1000)); return `${String(Math.floor(seconds / 60)).padStart(2, '0')}:${String(seconds % 60).padStart(2, '0')}`; }
function initials(name) { return String(name || '?').split(/\s+/).slice(0, 2).map(part => part[0] || '').join('').toUpperCase(); }
function podium(board) { return `<div class="podium">${(board || []).slice(0, 3).map(player => `<div class="podium-place"><div class="podium-avatar">${esc(initials(player.name))}</div><div class="podium-name">${esc(player.name)}</div><div class="podium-score">${player.score} Punkte</div><div class="podium-block">${player.rank}</div></div>`).join('')}</div>`; }
function boardHtml(board, title) { return `<div class="leaderboard-card"><div class="leaderboard-heading"><span class="eyebrow">Ahnsen Quizabend</span><h1>${esc(title)}</h1><p class="muted">${title === 'Endergebnis' ? 'Herzlichen Glückwunsch!' : 'Der aktuelle Stand nach der letzten Frage'}</p></div>${podium(board)}<div class="table-wrap"><table class="leaderboard"><thead><tr><th>Platz</th><th>Name</th><th>Punkte</th><th>Richtig</th></tr></thead><tbody>${(board || []).map(player => `<tr><td>${player.rank}</td><td>${esc(player.name)}</td><td><strong>${player.score}</strong></td><td>${player.correct}</td></tr>`).join('')}</tbody></table></div></div>`; }
function questionImageHtml(question) { const url = String(question?.imageUrl || '').trim(); return url ? `<img class="question-image" src="${esc(url)}" alt="Optionales Bild zur Quizfrage" loading="lazy">` : ''; }
...
function render(nextState) {
  ...
  if (nextState.phase === 'paused') {
    ...
    content.innerHTML = `<div class="screen-question-layout"><section class="screen-question-stage screen-question"><div class="screen-question-header"><div class="row"><span class="info-chip">${esc(nextState.progressLabel)}</span><span class="category-chip">${esc(question.category)}</span></div></div>${questionImageHtml(question)}<h1>${esc(question.text)}</h1><div class="screen-answers">${question.options.map((option, index) => `<div class="screen-answer ${nextState.phase === 'revealed' && question.correctIndex === index ? 'correct' : ''}"><span class="answer-letter">${String.fromCharCode(65 + index)}</span><span>${esc(option)}</span></div>`).join('')}</div>${nextState.phase === 'revealed' ? `<div class="screen-reveal-grid"><div class="screen-distribution-panel"><span class="eyebrow">Live-Auswertung</span><h2>Antwortverteilung</h2>${distributionHtml(nextState.distribution)}</div><div class="screen-distribution-panel"><span class="eyebrow">Richtige Antwort</span><h2 style="font-size:2rem;color:var(--green)">${String.fromCharCode(65 + question.correctIndex)} – ${esc(question.options[question.correctIndex])}</h2><p class="muted">${nextState.responseCount} Antworten wurden gewertet.</p></div></div>` : `<div class="screen-response-line"><span>${nextState.responseCount} von ${nextState.activePlayerCount} geantwortet</span><div class="screen-response-progress"><span style="--width:${answeredPercent}%"></span></div><strong>${answeredPercent}%</strong></div>`}</section><aside class="screen-timer-panel"><span class="eyebrow">Verbleibende Zeit</span><div id="screenTimer" class="timer-ring"><strong id="screenTimerText">${nextState.questionDurationSec}</strong></div><p class="muted">${nextState.phase === 'revealed' ? 'Frage beendet' : 'Sekunden'}</p></aside></div>`;
  } else if (nextState.phase === 'ready') {
    content.innerHTML = `<div class="big-status"><div class="icon">🎯</div><span class="eyebrow">Nächste Runde</span><h2>${esc(nextState.progressLabel)} ist bereit</h2><p class="muted" style="font-size:1.35rem">${nextState.activePlayerCount} Spieler sind verbunden.</p></div>`;
  } else {
    content.innerHTML = `<div class="screen-waiting"><span class="eyebrow">Willkommen</span><h1>${esc(nextState.title)}</h1><p class="muted" style="font-size:1.4rem">Handy öffnen, Adresse aufrufen und anmelden</p><div class="screen-url">${location.origin}</div><p class="muted">Teilnehmer im Warteraum: <strong style="color:white">${nextState.activePlayerCount}</strong></p></div>`;
  }
  const key = `${nextState.question?.id || ''}:${nextState.questionStartedAt || ''}`;
  if (key !== lastQuestionKey) { lastQuestionKey = key; beeped = new Set(); }
  clearInterval(timer);
...
function tick() {
  if (!state) return;
  if (state.phase === 'paused') { const node = document.querySelector('#screenPauseClock'); if (node) node.textContent = clock((state.pause?.until || 0) - (Date.now() + clockOffsetMs)); return; }
  ...
  if (seconds <= 5 && seconds >= 1 && !beeped.has(seconds)) { beeped.add(seconds); beep(seconds); }
}
fullscreenButton.addEventListener('click', async () => { unlockAudio(); try { if (!document.fullscreenElement) await document.documentElement.requestFullscreen(); else await document.exitFullscreen(); } catch {} });
