...
function installPhase1213Routes(app, { requireProfile, requireAdmin }) {
  ...
  app.get('/legal/guardian', wrap(async (req, res) => {
    try {
      ...
      res.type('html').send(legalPages.guardianResult(storage.legalConfig(), {
        ok: true,
        message: 'Die Zustimmung wurde gespeichert. Das Kinderprofil kann nun Community- und Wettbewerbsfunktionen verwenden.',
      }));
    } catch (error) {
  ...
  app.post('/api/platform/questions/report', requireProfile, wrap(async (req, res) => {
    res.status(201).json(await storage.reportQuestion(req.soloProfile.id, req.body || {}));
  }));
  ...
  app.patch('/api/platform/phase13/preferences', requireProfile, wrap(async (req, res) => {
    res.json({ preferences: await storage.updateEngagementPreferences(req.soloProfile.id, req.body || {}) });
  }));

  app.get('/api/platform/admin/phase12/release', requireAdmin, wrap(async (_req, res) => {
    res.json({ current: await storage.releaseChecks(), history: await storage.releaseHistory(), backups: await storage.backupChecks() });
  }));
  ...
  app.get('/api/platform/admin/phase12/question-reports', requireAdmin, wrap(async (req, res) => {
    res.json({ reports: await storage.questionReports(req.query.status || 'open') });
  }));
  ...
  app.get('/api/platform/admin/phase12/questions/stats', requireAdmin, wrap(async (req, res) => {
    res.json({ questions: await storage.questionStatistics(req.query.q || '') });
  }));
  ...
  app.get('/api/platform/admin/phase12/questions/:id/versions', requireAdmin, wrap(async (req, res) => {
    res.json({ versions: await storage.questionVersions(req.params.id) });
  }));
